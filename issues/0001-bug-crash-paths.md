...
6. 不正 UTF-8 を含むキー名
   ...
   - キーが 256 バイト以上（atom の最大長超過）の場合は badarg ではなく system_limit を投げる（実測: 300 バイトのキー）。0005 のバイナリ比較 / binary_to_existing_atom + catch で解消される見込みだが、完了条件にも含める
...
- kvconf_pkix の 3 関数は file:read_file/1 が例外を投げないため try-catch は不要（現状の {error, Reason} 分岐で error を返せる）。非パス入力を回帰テストで確認する
...
- 上記 6 経路の入力で initialize がクラッシュせず {error, ...} を返す
  ...
  - 経路 6 は {error, {invalid_key_name, Key, LineNumber}} を返す（キー長 256 バイト以上で発生する system_limit もクラッシュせずエラーを返す。0005 の実装後に確認する）
- 各経路の回帰テストが追加されている
