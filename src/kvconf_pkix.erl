...
validate_pkix_fullchain_pem_file(FullchainPemFilePath) ->
    maybe
        {ok, Bin} ?= file:read_file(FullchainPemFilePath),
        %% PEM or DER
        PemEntryList = public_key:pem_decode(Bin),
        true ?= PemEntryList =/= [],
...
validate_pkix_cert_pem_file(CertPemFilePath) ->
    maybe
        {ok, Bin} ?= file:read_file(CertPemFilePath),
        %% Format
        PemEntryList = public_key:pem_decode(Bin),
        true ?= is_list(PemEntryList) andalso PemEntryList =/= [],
        %% PemEntry が全て {'Certificate', _, not_encrypted} であることを確認する
        F = fun({'Certificate', Der, not_encrypted}) ->
                    try
                        %% 証明書がデコードできるか確認する
